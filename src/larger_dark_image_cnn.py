...
class Large_Dark_Image_CNN:
	...
	def build_model(self):
		...
		model.compile(optimizer=keras.optimizers.Adam(lr=.0005, decay=1e-5), loss='mean_squared_error', metrics=['mean_squared_error'])
		#print(model.summary())
		return model
	...
	def fit_model(self):
		self.model.fit_generator(self.generate_arrays_from_file('../new_train.txt'), 
			steps_per_epoch=math.ceil(self.num_training_samples/self.batch_size), epochs=100,
...
if __name__=='__main__':
	...
	with tf.device('/cpu:0'):
		cnn = Large_Dark_Image_CNN(batch_size,100)
